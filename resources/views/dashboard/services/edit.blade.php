...
@section('content')
    <form class="card shadow mb-4" action="{{ route('dashboard.services.update', $service->id) }}" method="POST"
        enctype="multipart/form-data" id="updateServiceForm">
        @csrf
        @method('PATCH')
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">{{ $service->title_ar }}</h6>
        </div>
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="form-group prev">
                        <img src="{{ asset('storage/' . $service->image) }}" style="width: 100px" class="img-thumbnail preview-formFile" alt="">
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="form-group col-6">
                    ...
                    <textarea class="form-control" name="desc_ar" id="" cols="30" rows="10" required>{{ old('desc_ar', $service->desc_ar) }}</textarea>
                </div>
                <div class="form-group col-6">
                    <label for="desc_en">المحتوى بالإنجليزية</label>
                    <textarea class="form-control" name="desc_en" id="" cols="30" rows="10" required>{{ old('desc_en', $service->desc_en) }}</textarea>
                </div>
            </div>
