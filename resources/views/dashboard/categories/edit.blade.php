...
@section('content')
    <form class="card shadow mb-4"
        action="{{ route('dashboard.categories.update', ['sectionSlug' => request()->sectionSlug, 'category' => $category->slug]) }}"
        method="POST" enctype="multipart/form-data" id="updateCategoryForm">
        @csrf
        @method('PATCH')
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">{{ $category->name_ar }}</h6>
        </div>
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="form-group prev">
                        <img src="{{ asset('storage/' . $category->img) }}" style="width: 100px"
            ...
            <div class="row">
                <div class="form-group col-6">
                    ...
                    <textarea class="form-control" name="desc_ar" id="" cols="30" rows="10" required>{{ old('desc_ar', $category->desc_ar) }}</textarea>
                    @error('desc_ar')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="form-group col-6">
                    ...
                    <textarea class="form-control" name="desc_en" id="" cols="30" rows="10" required>{{ old('desc_en', $category->desc_en) }}</textarea>
                    @error('desc_en')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
