...
@section('content')
    <form class="card shadow mb-4" action="{{ route('dashboard.vaccinations.update', $vaccination->slug) }}" method="POST"
        enctype="multipart/form-data" id="updateArticleForm">
        @csrf
        @method('PATCH')
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">{{ $vaccination->name_ar }}</h6>
        </div>
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="form-group prev">
                        <img src="{{ asset('storage/' . $vaccination->img) }}" style="width: 100px"
                    ...
                    <select name="article_id" class="form-control" id="" required>
                        ...
                        @foreach ($articles as $article)
                            <option value="{{ $article->id }}"
                                {{ old('article_id') == $article->id || $vaccination->articles->contains('id', $article->id) ? 'selected' : '' }}>
                                {{ $article->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                <div class="form-group col-6">
                    ...
                    <input type="text" class="form-control" name="name_ar" autocomplete="name_ar"
                        value="{{ old('name_ar', $vaccination->name_ar) }}" required>
                ...
                <div class="form-group col-6">
                    ...
                    <input type="text" class="form-control" name="name_en" autocomplete="name_en"
                        value="{{ old('name_en', $vaccination->name_en) }}" required>
            ...
            <div class="row">
                <div class="form-group col-6">
                    ...
                    <textarea class="form-control" name="desc_ar" id="desc_ar" cols="30" rows="10">{{ old('desc_ar', $vaccination->desc_ar) }}</textarea>
                    @error('desc_ar')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="form-group col-6">
                    ...
                    <textarea class="form-control" name="desc_en" id="desc_en" cols="30" rows="10">{{ old('desc_en', $vaccination->desc_en) }}</textarea>
                    @error('desc_en')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
