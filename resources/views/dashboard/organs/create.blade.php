...
@section('content')
    ...
    <form class="card shadow mb-4" action="{{ route('dashboard.organs.store') }}" method="POST" enctype="multipart/form-data"
        id="createOrganForm">
        @csrf
        ...
        <div class="card-body">
            ...
            <div class="row">
                <div class="form-group col-12">
                    <label for="body_system_id">الجهاز</label>

                    <select name="body_system_id" class="form-control" id="" required>
                        <option value="">اختر</option>
                        @foreach ($bodySystems as $system)
                            <option value="{{ $system->id }}"
                                {{ old('body_system_id') == $system->id ? 'selected' : '' }}>
                                {{ $system->name }}</option>
                        @endforeach
                    </select>
